...
void recc(Node* temp,GraphList* graph,Stack* stack, int *arr) {
    if(temp==NULL) return;
    int adj_vertex = temp->vertex;
    if(!arr[adj_vertex]) {
                push(stack, adj_vertex);
    ...
    temp = temp->next;
    return recc(temp,graph,stack,arr);
}
void traverse_rec(GraphList* graph, int start_vertex, Stack* stack, int curr_vertex,int* arr) {
    ...
    return traverse_rec(graph,start_vertex,stack,curr_vertex,arr);
